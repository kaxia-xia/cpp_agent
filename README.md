...
> 💡 **控制台编码与颜色**：coding-agent 启动时会自动把控制台输出/输入代码页设为 **UTF-8**（等价于 `chcp 65001`），并开启 **ANSI 虚拟终端（VT）处理**，因此中文不会乱码、彩色输出正常渲染，无需手动操作。若终端不支持 VT（如极老的控制台），程序会自动降级为纯文本输出，不会出现 `35m` 这类转义序列残留。
